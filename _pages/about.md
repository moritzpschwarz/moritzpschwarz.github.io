...
In my research, I make ample use of econometric time series and panel methods and contribute to the development of open-source estimation software, such as the [{gets} R-package](https://cran.r-project.org/web/packages/gets/index.html) and the [{getspanel} R-package](getspanel). More recently, I have been developing the Open Source Empircial Macro Model (OSEM) as well as the associated R package [{osem}](osem) together with co-authors.
